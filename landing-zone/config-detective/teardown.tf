...
check "destroy_guard_still_on" {
  assert {
    ...
    error_message = "allow_destroy = true: bucket log AWS Config dang o che do force_destroy. Dung neu dang teardown. Xong viec - hoac neu doi y - dat lai false va apply."
  }
}
