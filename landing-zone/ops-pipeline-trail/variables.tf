...
variable "enable_trail_stage" {
  description = <<-EOT
    Bat stage cloudops-trail.

    CAN TRUOC: role rieng o ACCOUNT LOG-ARCHIVE - org-trail/versions.tf:69
    assume sang do.

    LUU Y VE NHIP DO: CloudTrail doi vai lan mot nam. Gia tri chinh cua
    pipeline nay khong phai apply ma la DRIFT - CodeBuild drift chay
    `plan -lock=false` hang dem, va mot lan ra KHAC "khong co thay doi"
    nghia la co nguoi vua cham vao thu duy nhat tra loi duoc "ai da lam
    gi".
  EOT
  type        = bool
  default     = false
...
variable "trail_pipeline_role_arns" {
  description = "ARN role pipeline nay duoc assume sang account LOG-ARCHIVE. Rong la mac dinh. DUNG dien OrganizationAccountAccessRole."
  type        = list(string)
  default     = []
}
