...
check "destroy_guard_still_on" {
  assert {
    ...
    error_message = "allow_destroy = true: Network Firewall dang TAT delete_protection va subnet_change_protection, va bucket log dang o che do force_destroy. Dung neu dang teardown. Xong viec - hoac neu doi y - dat lai false va apply."
  }
}
