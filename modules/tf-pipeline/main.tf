...
########################################
# PIPELINE PHAI CO VERIFY, HOAC MOT LY DO VI SAO KHONG
#
# Cung khuon voi co_catalog_de_lint, nhung bat mot khoang trong KHAC:
#
#   lint    kiem TRUOC khi Terraform cham vao AWS  - "co nen lam khong"
#   verify  doc lai AWS SAU khi apply xong         - "da lam duoc chua"
#
...
########################################
check "co_verify_sau_apply" {
  assert {
    condition     = var.verify != "" || var.khong_co_verify != ""
    error_message = join(" ", [
      "var.verify RONG va cung khong khai khong_co_verify.",
      "Rong la hop le - co pipeline that su khong verify duoc tu account",
      "management - nhung no phai duoc VIET RA kem ly do. Mot truong bo trong",
      "doc giong het mot truong bi quen, va cai thu hai la mot pipeline apply",
      "xong ma khong ai hoi lai AWS xem no co tac dung khong.",
    ])
  }
}
